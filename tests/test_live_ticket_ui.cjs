...
function harness(){
 ...
 function node(id){return nodes[id]??=( {id,value:'',disabled:false,hidden:false,textContent:'',handlers:{},dataset:{},focus(){this.focused=true},scrollIntoView(){},addEventListener(n,f){this.handlers[n]=f}} );}
 const radio={};
 ...
 const context={document:{getElementById:node,addEventListener:(n,f)=>handlers[n]=f},window:{addEventListener:(n,f)=>windows[n]=f,dispatchEvent(){}},
...
(async()=>{
 ...
 const late=h.preview();h.node('live-ticket-shares').value='3';h.node('live-ticket-form').handlers.input();h.resolve(2,h.response('late'));await late;
 ...
 assert.equal(h.radio['live-ticket-intent'].selected,'cover');assert.equal(h.node('live-ticket-shares').value,'3');assert.equal(h.requests.length,count);
 h.node('live-ticket-symbol').value='TEST';const failed=h.preview();h.resolve(7,h.response('failed'));await failed;h.ack('TEST');
